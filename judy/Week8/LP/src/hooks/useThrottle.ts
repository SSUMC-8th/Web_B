...
function useThrottle<T>(value: T, delay = 500) {
  ...
  // 2. Ref lastExcecuted: 마지막으로 실행된 시간을 기록하는 함수
  // useRef는 사용하면 컴포넌트가 리렌더링 되어도 값이 유지되고, 변경되어도 리렌더링을 트리거하지 않아요
  const lastExcecuted = useRef<number>(Date.now());

  // 3. useEffect: value, delay가 변경될 때 아리 로직 실행
  useEffect(() => {
    // 현재 시간과 lastExcucted.current에 저장된 마지막 시간 + delay을 비교
    // 충분한 시간이 지나면 바로 업데이트
    if (Date.now() >= lastExcecuted.current + delay) {
      ...
      // 최신 value를 throttledValue 저장해서 컴포넌트 리렌더링
